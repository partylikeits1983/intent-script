# IntentOS JSON DSL Specification — V1

> This document specifies the JSON format accepted by the intent-script compiler.
...
Each step is a JSON object with exactly one key (the action name) mapping to its parameters.
...
*Either `min_amount_out` or `price` must be provided. Zero slippage is rejected.
...
The `"all"` keyword resolves at compile time to the guaranteed minimum output of the most recent prior step that produces the same token. It cannot be used on the first step or when no prior step produces the token.
...
2. **Steps**: 1–5 steps required (max 5)
3. **Amounts**: Must be positive (> 0)
4. **Slippage**: Swaps must have `min_amount_out` or `price`+`slippage`
